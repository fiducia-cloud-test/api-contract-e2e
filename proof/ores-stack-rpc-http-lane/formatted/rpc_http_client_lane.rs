...
fn typescript_source(
    ...
    let canonical =
        canonical_relative_import(RpcSdkLanguage::TypeScript, namespace, operation_name);
...
fn rust_descriptor(contract: &RpcOperationContract, digest: &str) -> String {
    format!(
        ...
        contract
            ...
            .iter()
            .map(|codec| format!(
                "::ores_http_clients::RpcPayloadCodecName::{}",
                rust_codec(*codec)
            ))
            .collect::<Vec<_>>()
            .join(", "),
...
fn add_rust_indexes(
    ...
    for (namespace, mut node) in nodes {
        ...
        let mut content = String::from(
            "// @generated by ores-stack parallel ores-http-clients lane; DO NOT EDIT.\n",
        );
...
fn write_or_check(target: &Path, path: &Path, content: &str, check: bool) -> Result<()> {
    if !path.starts_with(target) {
        bail!(
            "parallel RPC output {} escapes target {}",
            path.display(),
            target.display()
        );
    }
    ...
    if let Ok(metadata) = fs::symlink_metadata(path) {
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            bail!(
                "parallel RPC output {} must be a regular file",
                path.display()
            );
        }
    }
