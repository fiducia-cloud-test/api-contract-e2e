...
const unaryOperation: RpcOperationDescriptor = {
  operationKey: "demo.health.get_version",
  endpoint: "/v1/rpc",
  streamMode: "unary",
  allowedCodecs: ["json", "messagepack"],
  defaultCodec: "json",
...
class DemoClient extends OresRpcClientBase {
  ...
  getVersion(input: {id: string}) {
    return this.unary(unaryOperation, input, async (_operation, value, options) => {
      this.calls += 1;
  ...
  watch() {
    return this.serverStream(streamOperation, {}, async (_operation, _input, options) => {
      this.opens += 1;
      this.lastOptions = options;
      const values = [1, 2];
      const handle: RpcStreamHandle<number> = {
        async cancel() {},
